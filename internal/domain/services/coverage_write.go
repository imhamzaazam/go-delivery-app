...
func (service *CommerceManager) CreateAreaHTTP(ctx context.Context, viewerActorID uuid.UUID, viewerMerchantID uuid.UUID, name string, city string) (pgsqlc.Area, *domainerr.DomainError) {
...
func (service *CommerceManager) CreateZoneHTTP(ctx context.Context, viewerActorID uuid.UUID, viewerMerchantID uuid.UUID, areaID string, name string, coordinatesWKT string) (pgsqlc.CreateZoneRow, *domainerr.DomainError) {
	allowed, allowErr := service.canViewMerchant(ctx, viewerActorID, viewerMerchantID)
	if allowErr != nil {
