...
func (service *CommerceManager) SalesReportByMonth(ctx context.Context, viewerActorID uuid.UUID, merchantID uuid.UUID, month int, year int) (SalesReport, *domainerr.DomainError) {
	...
	report.TotalSales = round2(report.TotalSales)
	report.TotalTax = round2(report.TotalTax)
	report.TotalDiscount = round2(report.TotalDiscount)
	report.ProfitEstimate = round2(report.ProfitEstimate)

	return report, nil
}

func (service *CommerceManager) ViewInventory(ctx context.Context, viewerActorID uuid.UUID, merchantID uuid.UUID) ([]InventoryItem, *domainerr.DomainError) {
	allowed, allowErr := service.canViewMerchant(ctx, viewerActorID, merchantID)
	if allowErr != nil {
...
func (service *CommerceManager) canViewMerchant(ctx context.Context, actorID uuid.UUID, merchantID uuid.UUID) (bool, error) {
	isAdmin, adminErr := service.hasRole(ctx, actorID, pgsqlc.RoleTypeAdmin, uuid.Nil)
	if adminErr != nil {
	...
	isMerchant, merchantErr := service.hasRole(ctx, actorID, pgsqlc.RoleTypeMerchant, merchantID)
	if merchantErr != nil {
